...
class hikashopUserClass extends hikashopClass {
	...
	public function getPrivacyConsentSettings() {
		$pluginsClass = hikashop_get('class.plugins');
		$plugin = $pluginsClass->getByName('system','privacyconsent');

		if(!$plugin->enabled)
			return false;

		$language = JFactory::getLanguage();
		$language->load('plg_system_privacyconsent', JPATH_SITE, $language->getTag(), true);

		$privacyArticleId = @$plugin->params['privacy_article'];
		$privacyNote = @$plugin->params['privacy_note'];
		if(empty($privacyNote))
			$privacyNote = JText::_('PLG_SYSTEM_PRIVACYCONSENT_NOTE_FIELD_DEFAULT');
