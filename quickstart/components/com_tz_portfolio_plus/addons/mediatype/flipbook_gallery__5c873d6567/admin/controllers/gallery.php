...
class TZ_Portfolio_Plus_Addon_Flipbook_GalleryControllerGallery extends TZ_Portfolio_PlusControllerAddon_Datas{
    ...
    public function ajax()
    {
	    ...
	    try {
		    ...
		    if (JFolder::create($tmp_resize_folder)) {
			    $addon      =   TZ_Portfolio_PlusPluginHelper::getPlugin('mediatype','flipbook_gallery');
			    ...
			    if ($params && $image_size = $params->get('flipbook_gallery_size')) {
				    ...
				    $flipbook_gallery   =   new JImage();

				    $flipbook_gallery -> destroy();
				    $flipbook_gallery -> loadFile($tmp_dest);
				    ...
				    foreach ($image_size as $_size) {
					    ...
					    $imageProperties   = $flipbook_gallery->getImageFileProperties($tmp_dest);
					    $newH              = ($imageProperties->height * $size->width) / ($imageProperties->width);
					    $newImage          = $flipbook_gallery->resize($size->width, $newH);
	...
	public function delete() {
        ...
        try {
            ...
            if ($source == 'client') {
                ...
                $folder             =   JPATH_ROOT.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'tz_portfolio_plus'.DIRECTORY_SEPARATOR.'flipbook_gallery'.DIRECTORY_SEPARATOR.$article_id;
            }
